...
export default function BeerGuessGame() {
  ...
  const [hintValidation, setHintValidation] = useState<GuessHistoryItem['validation'] | null>(null)
  ...
  const getHintClass = (isCorrect: boolean) =>
    `px-4 py-1 rounded text-sm font-semibold ${
      isCorrect ? 'bg-green-100 text-green-800 border border-green-400' : 'bg-red-100 text-red-800 border border-red-400'
    }`

  if (!answer) return <p className="text-center mt-20 text-lg">Loading beer… 🍺</p>

  return (
    <div className="max-w-xl mx-auto mt-12 p-6 bg-white shadow-md rounded-lg">
      <h1 className="text-2xl font-bold mb-6 text-center">🍺 Beer Guess Game</h1>
      ...
      {hintValidation && (
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            ...
            <div className={getHintClass(hintValidation.country)}>Country</div>
          ...
          <ul className="space-y-1 text-sm">
            {guessHistory.map((item, idx) => (
              <li key={idx} className="border-b pb-1">
                ...
                <span className={item.guess.name === answer.name ? 'text-green-600' : 'text-red-600'}>
                  {item.guess.name === answer.name ? 'Correct!' : 'Wrong'}
                </span>{' '}
                | Style:{' '}
                <span className={item.validation.style ? 'text-green-600' : 'text-red-600'}>
                  {item.validation.style ? '✔' : '✘'}
                </span>{' '}
                | Country:{' '}
                <span className={item.validation.country ? 'text-green-600' : 'text-red-600'}>
                  {item.validation.country ? '✔' : '✘'}
                </span>{' '}
                | Emoji:{' '}
                <span className={item.validation.emoji ? 'text-green-600' : 'text-red-600'}>
